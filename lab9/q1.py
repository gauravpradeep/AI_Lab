def waterJug_bfs(j1_cap, j2_cap, target_amount):   
    j1 = 0  
    j2 = 0  
    ...
    while queue: 
        ...
        if (j1, j2) not in visited:
        	...
        	for action in actions:
        		if action[0] == "fill":  
		            if action[1] == 1:
		            	next_state = (j1_cap, j2)
		            else:
		            	next_state = (j1, j2_cap)
		        	...
		        	if action[1] == 1:
		        		amount = min(j1, j2_cap - j2)
		        		next_state = (j1 - amount, j2 + amount)
		        	else:
		        		amount = min(j2, j1_cap - j1)
		        		next_state = (j1 + amount, j2 - amount)  
